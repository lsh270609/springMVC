...
public class TestController {
	...
	public String getResult1(Model model) {
		...
		//model.addAttribute()으로 data 를 메모리에 올리면 request scope 에 저장됨 <- result1.jsp 에서 requestScope로 데이터 가져와야함
	...
	public String getResult2(Model model) {
		...
		//@Component를 사용하는 ByType 방식은 application scope 자동으로 저장되지 않아서 model 로 request scope 에 올림
		//@Component를 사용하는 ByName 방식은 application scope 자동으로 저장되지 않아서 model 로 request scope 에 올림
